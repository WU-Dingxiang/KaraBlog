...
import top.karablog.model.Log;
import top.karablog.util.DbUtil;

public class SeeWaterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		// 2- 获得对应user id的数据条目数
		Long logCount = DbUtil.countLog(userid, "water");
		...
		// 3- 获得对应区间的数据
		List<Log> result = DbUtil.seeLog(userid, pageIndex, "water");
	...
	private StringBuffer makeRow(StringBuffer buffer, Long pageIndex,
		...
		buffer.append("\" onclick=\"$.goWaterPage(");
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, java.io.IOException {
		System.out.println("SeeWaterServlet : back to 'http://localhost:8080/karablog'");
		resp.sendRedirect("/");
	}
