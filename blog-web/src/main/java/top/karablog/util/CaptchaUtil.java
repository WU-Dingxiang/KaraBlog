...
public class CaptchaUtil {
	public static final String KEY = new Date().toString();
	...
	public static boolean isCaptchaValid(HttpServletRequest request) {
		...
		try {
			server_captcha = DesUtil.decrypt(captcha_des, KEY);// 验证码解码
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.out.println("CaptchaUtil.server_captcha = " + server_captcha);

		boolean captchaValid = server_captcha != null && server_captcha.equals(captcha);
		return captchaValid;
	}
}
